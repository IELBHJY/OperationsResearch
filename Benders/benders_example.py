...
class Benders_Example(object):
    ...
    def creat_dual_model(self,input_y_i,iteration):
        ...
        v_i = model.addVars(self.i_num, obj=sum(self.d) * input_y_i,vtype=GRB.CONTINUOUS,lb=-GRB.INFINITY,ub=0,name='v')
        w_j = model.addVars(self.j_num, obj=self.d, vtype=GRB.CONTINUOUS,name="w")
        for index in range(len(input_y_i)):
            model.addVar(lb=input_y_i[index],ub=input_y_i[index],obj=self.f[index],vtype=GRB.CONTINUOUS,name='y_'+str(index))

        #add cons
        model.addConstrs((v_i[i] + w_j[j] <= self.cost[i][j] for i in range(self.i_num) for j in range(self.j_num)),name='c')
